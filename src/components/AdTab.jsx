...
function charMeta(text, limit) {
  ...
  const color = over ? 'rgba(220,70,70,.9)' : warn ? 'rgba(220,160,0,.85)' : 'var(--text-3)'
...
export default function AdTab({ showToast }) {
  ...
  const generate = useCallback(async () => {
    ...
    setGenerating(true)
    try {
      const system = AD_SYSTEM(state.globalContext, objective, placement, funnel)
  ...
  }, [state, selectedImg, objective, placement, funnel, showToast])
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr 296px', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '8px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-raised)', flexShrink: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 9, color: 'var(--text-2)', fontFamily: 'var(--font-mono)', letterSpacing: '.08em' }}>
            {OBJECTIVES.find(o => o.id === objective)?.label}
          </span>
          <span style={{ color: 'var(--border)', fontSize: 10 }}>·</span>
          <span style={{ fontSize: 9, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>{activePlacement?.label}</span>
          <span style={{ color: 'var(--border)', fontSize: 10 }}>·</span>
          <span style={{ fontSize: 9, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>{FUNNEL.find(f => f.id === funnel)?.label}</span>
          {variants.length > 0 && (
                  ...
                  <div key={i} onClick={() => setSelectedIdx(i)}
                    ...
                    <div style={{ padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        <span style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)' }}>#{i + 1}</span>
                      </div>
                      ...
                      {[
                        ...
                        <div key={label}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                            <span style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '.1em', textTransform: 'uppercase' }}>{label}</span>
                            <span style={{ fontSize: 7, fontFamily: 'var(--font-mono)', color: meta.color }}>{meta.len}/{limit}</span>
                          </div>
                          <div style={{ fontSize: 10, color: meta.over ? 'rgba(220,90,90,.9)' : 'var(--text-2)', fontFamily: 'var(--font-body)', lineHeight: 1.45 }}>{text}</div>
                        </div>
                      ))}

                      {/* CTA chip */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '.1em', textTransform: 'uppercase' }}>CTA</span>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg-raised)', borderLeft: '1px solid var(--border)' }}>
          ...
          {selectedImg && (
            <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 8 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 8, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', marginTop: 2 }}>{selectedImg.width}×{selectedImg.height}</div>
              </div>
            </div>
          )}
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 8 }}>Objective</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
              {OBJECTIVES.map(o => (
                <button key={o.id} onClick={() => setObjective(o.id)}
                  ...
                  <div style={{ fontSize: 9, fontWeight: 600, color: objective === o.id ? 'var(--silver)' : 'var(--text-2)', fontFamily: 'var(--font-mono)' }}>{o.label}</div>
                  <div style={{ fontSize: 7, color: 'var(--text-3)', marginTop: 1 }}>{o.desc}</div>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Placement */}
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 8 }}>Placement</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
              {PLACEMENTS.map(p => (
                <button key={p.id} onClick={() => setPlacement(p.id)}
                  style={{ padding: '6px 8px', fontSize: 9, fontFamily: 'var(--font-mono)', background: placement === p.id ? 'var(--silver-ghost)' : 'none', border: `1px solid ${placement === p.id ? 'var(--silver-edge)' : 'var(--border)'}`, borderRadius: 'var(--r)', color: placement === p.id ? 'var(--silver)' : 'var(--text-3)', cursor: 'pointer', lineHeight: 1.3 }}>
          ...
          {/* Funnel stage */}
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 8 }}>Audience Temperature</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {FUNNEL.map(f => (
                <button key={f.id} onClick={() => setFunnel(f.id)}
                  ...
                  <span style={{ fontSize: 9, fontWeight: 600, color: funnel === f.id ? 'var(--silver)' : 'var(--text-2)', fontFamily: 'var(--font-mono)', minWidth: 36 }}>{f.label}</span>
                  <span style={{ fontSize: 8, color: 'var(--text-3)', lineHeight: 1.3 }}>{f.desc}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div style={{ padding: '10px 14px' }}>
            <div style={{ fontSize: 7, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', letterSpacing: '.12em', textTransform: 'uppercase', marginBottom: 8 }}>Meta Ads Manager</div>
            {!state.settings.adAccountId ? (
              <div style={{ fontSize: 8, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', lineHeight: 1.6 }}>
              ...
              <div style={{ fontSize: 8, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', lineHeight: 1.5 }}>
